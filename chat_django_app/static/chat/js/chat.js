...
function update_chat_log(full_update){
	...
	$.get('/chat_api', dataToBeSent, function(data, textStatus) {
...
function update_classification(message_id, classification,
    ...
    for (var i=previous_messages.length-1; i>-1; i--){
        concatenated_classification = classifications[i];
        for (var j=i; j>-1; j--){
            ...
            if (concatenated_classification != 'N'){ 
                if (previous_classification == 'N'){
                    comment = 'Relabeled due to ' + (i+2) + '-fold concatenation';
                    if (classification == 'N'){
                        set_class(message_id, concatenated_classification,
                        comment)
...
function verbose_classification(tag){
	switch(tag){
		case 'B': return 'BULLYING';
		case 'U': return 'UNCLASSIFIED';
		case 'R': return 'RACISM';
		case 'N': return 'NORMAL';
		case 'S': return 'SEXISM';
...
function post_message(){
	...
	$.post('chat_api', dataToBeSent, function(data, textStatus) {
	...
	setTimeout(function(){update_chat_log(false);}, 50);
